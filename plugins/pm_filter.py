...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("**Search for **{search} 🔎", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ᴀʙᴏᴜᴛ', callback_data='about')
             ...
             InlineKeyboardButton(f'ᴏᴛᴛ ᴜᴘᴅᴀᴛᴇs​', url='https://t.me/MRXSUPPORTS'),
             InlineKeyboardButton(f'ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ', url='https://t.me/MRXSUPPORTS')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
